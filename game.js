...
function setAudio(on){
  ...
  if(on){ initAudio(); if(running) startMusic(); }
  else stopMusic();
}
...
function makeEnemy(x,y,r,hp){
  return {x,y,r,hp,maxHp:hp,t:Math.random()*10,type:"enemy"};
}
function buildLevel(){
  bumpers = [
    {x:W*.25,y:H*.33,r:25},
  ...
  enemies = [
    makeEnemy(W*.22,H*.17,22,5),
    makeEnemy(W*.50,H*.23,24,6),
    makeEnemy(W*.78,H*.17,22,5),
  ];
  boss = {x:W*.5,y:H*.08,r:42,hp:18,maxHp:18,awake:false,type:"boss"};
  bossWasAwake=false;
  combo=0;
  comboTimer=0;
}
...
function update(dt){
  ...
  enemies.forEach(e=>{
    ...
    if(e.hp>0 && collideCircle(e,1.05,70)){
      ...
      if(e.hp<=0){
        score += 500;
        addDebris(e.x,e.y,"#ffde9d",24,6);
        addGlowBurst(e.x,e.y,20);
        sfx.down();
      }else{
        sfx.hit();
      }
  ...
  if(boss.awake && boss.hp>0 && collideCircle(boss,1.07,100)){
    ...
    if(boss.hp<=0){
      score += 5000;
      addDebris(boss.x,boss.y,"#ffd2d8",40,8);
      addGlowBurst(boss.x,boss.y,28);
      running=false;
      stopMusic();
      sfx.clear();
...
function drawBoard(){
  ...
  ctx.setLineDash([8,8]);
  ctx.beginPath();
  ctx.moveTo(W*.22,H*.135);
  ctx.lineTo(W*.78,H*.135);
  ctx.stroke();
  ctx.setLineDash([]);
}
...
function drawChippedShell(r, ratio, shellColor, coreColor){
  ...
  body.addColorStop(.12,shellColor.highlight);
  body.addColorStop(.48,shellColor.mid);
  body.addColorStop(1,shellColor.dark);
  ctx.fillStyle=body;
  ctx.beginPath();
  ctx.arc(0,0,r,0,Math.PI*2);
  ctx.fill();

  // exposed core depending on damage
  const damage = 1-ratio;
  if(damage>0.12){
    const core=ctx.createRadialGradient(-r*.18,-r*.12,2,0,0,r*.75);
    core.addColorStop(0,"#fff");
    core.addColorStop(.2,coreColor.light);
    core.addColorStop(1,coreColor.dark);
    ctx.fillStyle=core;
    ctx.beginPath();
    ctx.arc(-r*.05,r*.06,r*(.25 + damage*.18),0,Math.PI*2);
    ctx.fill();
  }

  // chip cutouts from outer shell
  ctx.save();
  ctx.globalCompositeOperation="destination-out";
  const chips = Math.floor(damage*8);
  for(let i=0;i<chips;i++){
    const a = -1.2 + i*0.72 + damage*0.8;
    const cr = r*(.18 + (i%3)*.03);
    const cx = Math.cos(a)*r*.8;
    const cy = Math.sin(a)*r*.8;
    ctx.beginPath();
  ...
  // rim
  ctx.strokeStyle=shellColor.rim;
  ctx.lineWidth=3;
  ctx.stroke();
...
function drawEnemy(e){
  ...
  drawChippedShell(
    e.r, ratio,
    {highlight:"#ffd08d", mid:"#d48a2b", dark:"#734112", rim:"#ffbd62"},
    {light:"#fff6c7", dark:"#ff8d2f"}
  );
  ctx.shadowBlur=0;
  ...
  ctx.fillText(String(e.hp),0,-e.r-10);
...
function drawBoss(){
  ...
  ctx.shadowBlur=boss.awake?22:8;
  ctx.shadowColor=boss.awake?"#ff536e":"#33475f";

  if(!boss.awake){
    const idle=ctx.createRadialGradient(-8,-8,4,0,0,boss.r);
    idle.addColorStop(0,"#5a6776");
    idle.addColorStop(1,"#263446");
    ctx.fillStyle=idle;
    ...
    drawChippedShell(
      boss.r, ratio,
      {highlight:"#ffb2be", mid:"#b8324c", dark:"#5b1524", rim:"#ff8ba0"},
      {light:"#ffffff", dark:"#8df1ff"}
    );
    // glowing exposed core gets larger as damage rises
    const damage = 1-ratio;
    const core=ctx.createRadialGradient(-5,-5,3,0,0,boss.r*.75);
    core.addColorStop(0,"#fff");
    core.addColorStop(.22,"#d8ffff");
    core.addColorStop(1,"#52ddff");
    ctx.fillStyle=core;
    ctx.globalAlpha = .2 + damage*.5;
    ctx.beginPath();
    ctx.arc(0,0,boss.r*(.18 + damage*.26),0,Math.PI*2);
    ctx.fill();
    ctx.globalAlpha = 1;
  }
  ...
  if(boss.awake){
    ...
    ctx.textAlign="center";
    ctx.fillText("AI CORE",0,-boss.r-14);
...
function drawBall(){
  ...
  orb.addColorStop(.24,"#b5fbff");
  orb.addColorStop(.55,"#42d7ff");
  orb.addColorStop(1,"#2454d8");
  ctx.fillStyle=orb;
  ctx.shadowBlur=22;
  ctx.shadowColor="#66ecff";
...
function drawHUDMessages(){
  if(!boss?.awake && running){
    ...
    ctx.fillText("3 DRONESを削り切って上層ゲートを開け",W/2,H*.14);
  }
  if(combo>1 && comboTimer>0){
    ctx.fillStyle="rgba(255,245,200,.95)";
    ctx.textAlign="center";
    ctx.font="900 22px system-ui";
    ctx.fillText(combo + " COMBO!", W/2, H*.61);
  }
}
function draw(){
